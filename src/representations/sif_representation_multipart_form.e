note
	description: "Summary description for {SIF_REPRESENTATION_MULTIPART_FORM}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"

class
	SIF_REPRESENTATION_MULTIPART_FORM
	inherit
		SIF_REPRESENTATION
...
feature -- Status
	...
	type: like {SIF_REPRESENTATION_ENUMERATION}.type
		...
		do
			Result := {SIF_REPRESENTATION_ENUMERATION}.multipart_form
		end
		...
		do
			...
			loop
				...
				if l_ie.is_mandatory then
					...
					if attached req.form_parameter (l_ie.descriptor) as l_form_item_mandatory then
					   	if l_ie.is_valid_input( l_form_item_mandatory.string_representation ) then
							l_ie.put_input (url_encoder.decoded_utf_8_string (l_form_item_mandatory.string_representation))
				...
				if l_ie.is_optional then
					if attached req.form_parameter (l_ie.descriptor) as l_from_item_optional then
					   	if l_ie.is_valid_input( l_from_item_optional.string_representation ) then
							l_ie.put_input (url_encoder.decoded_utf_8_string (l_from_item_optional.string_representation))
							a_ie_set_to_publish.extend (l_ie )
					   	end
					end
				end
